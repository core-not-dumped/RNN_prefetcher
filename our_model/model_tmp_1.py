...
def make_train_data_rp4(load_address):
    ...
    for i in range(len(address_delta)):
        ...
        if len(delta_list) < seq_length + output_length:
            continue
        
        for j in list(range(0, len(delta_list) - seq_length - output_length + 1)):
            ...
            for k in range(output_length):
                output_tmp[delta_list[j + seq_length + k]] = 1
    ...
    def generate(self, data):
        ...
        for i in tqdm(range(len(load_address))):
            ...
            while cnt < 2:
                ...
                cnt += 1
            prefetches_address = prefetches_address + access[load][:-2]
